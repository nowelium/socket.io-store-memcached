...
function Memcached (opts) {
  ...
  if(!opts.memcachedStorePubSubInterval){
    opts.memcachedStorePubSubInterval = 1000;
  }
  if(!opts.memcachedStoreEnableGetMulti){
    opts.memcachedStoreEnableGetMulti = true;
  }
...
Memcached.prototype.unsubscribe = function (name, fn) {
  this._pubsub.unsubscribe(name, fn || function (){});
...
function Client (store, id) {
  ...
  var options = this.store.options;
  var namespace = options.memcachedStoreNamespace;
  var client = this.store._client;
  this._keyList = new MemcachedList(client, util.format('%s/keys/%s', namespace, id), options);
  this._map = new MemcachedMap(client, util.format('%s/map/%s', namespace, id), options);
};
...
Client.prototype.destroy = function (expiration) {
  ...
  // {{{ TODO: socket.io/manager isLocal?
  this.store.unsubscribe('message:' + this.id);
  this.store.unsubscribe('disconnect:' + this.id);
  // }}} TODO
